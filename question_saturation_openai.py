...
------------------------------
  - "openai" : semantic embeddings via OpenAI text-embedding-3-small/large
               (RECOMMENDED — rich semantics, matches production RAG quality).
               Requires: $env:OPENAI_API_KEY = "sk-..."
               Switch between small/large via EMBED_MODEL below.
  - "tfidf"  : lexical vectors, no model/network/API key needed. Good for a
               quick local run. Less semantic — treats "annual leave" and
               "vacation" as unrelated.
...
# ── CONFIG ─────────────────────────────────────────────
EMBED_BACKEND  = "openai"                      # "openai" or "tfidf"
...
# least this value. Set to None to auto-calibrate from the similarity matrix.
...
def main(doc_path, questions_path, out_path="sahuration.png"):
    md = Path(doc_path).read_text()
    chunks = chunk_markdown_by_heading(md)
    questions = load_questions(Path(questions_path))

    backend_label = f"{EMBED_BACKEND} ({EMBED_MODEL})" if EMBED_BACKEND == "openai" else EMBED_BACKEND
...
if __name__ == "__main__":
    main(
        ...
        "sahuration.png"                        # output graph saved here
    )
